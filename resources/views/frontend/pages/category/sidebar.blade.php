<div class="service-details__sidebar" id="service-sidebar">
    <div class="service-details__sidebar-category">
        <h3 class="service-details__sidebar-title">Service Category</h3>
        <ul class="service-details__sidebar-category-list list-unstyled">
            @if(!empty($service_categories))
                @foreach($service_categories as $index => $latest)
                <li class="@if(Request::url() === url('/our-category/'.$latest->slug)) active @endif "><a   href="{{route('service.single',$latest->slug)}}">{{ucwords(@$latest->name)}}<span
